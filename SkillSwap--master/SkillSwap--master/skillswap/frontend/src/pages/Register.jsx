...
const Register = () => {
  ...
  return (
    ...
    >
      <motion.div
        className={`max-w-md w-full rounded-3xl shadow-2xl p-10 border ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'
          }`}
      ...
      >
        <motion.div variants={containerVariants} initial="hidden" animate="show">
          ...
          <motion.p variants={itemVariants} className={`text-center text-sm mb-6 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          ...
          {referralCode && (
            <motion.div
              ...
              className="bg-blue-100/90 dark:bg-blue-900/30 border border-blue-300 text-blue-800 dark:text-blue-300 px-4 py-3 rounded-xl mb-6 text-sm font-medium"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <motion.div variants={itemVariants}>
              <motion.label className="block text-sm font-semibold mb-2 inline-block" whileHover={{ x: 4 }}>
              ...
              <motion.input
                ...
                className={`w-full px-4 py-2.5 rounded-xl border-2 focus:outline-none transition-all ${isDark
                ...
                whileFocus={{
                  scale: 1.01,
                  boxShadow: isDark ? '0 0 15px rgba(34, 211, 238, 0.15)' : '0 0 15px rgba(59, 130, 246, 0.15)',
                }}
                required
              />
            </motion.div>
            ...
            <motion.button
              ...
              type="submit"
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-blue-600 via-blue-500 to-cyan-500 text-white py-3 rounded-xl font-bold text-lg hover:opacity-90 disabled:opacity-60 disabled:cursor-not-allowed transform transition-all duration-200 soft-pulse gradient-glow ripple-effect shine-sweep smooth-transform relative overflow-hidden shadow-lg mt-2"
          ...
          <motion.div variants={itemVariants} className="mt-5 space-y-3">
            <motion.button
              ...
              className="w-full bg-white dark:bg-gray-700 text-gray-800 dark:text-white py-2.5 rounded-xl border-2 border-gray-300 dark:border-gray-600 font-bold text-lg hover:bg-gray-50 dark:hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed transform transition-all duration-200 ripple-effect smooth-transform"
              whileHover={{ scale: isGoogleLoading ? 1 : 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              <span className="flex items-center justify-center gap-2">
                🔵 {isGoogleLoading ? 'Connecting...' : 'Google Sign Up'}
              </span>
            </motion.button>
          </motion.div>
